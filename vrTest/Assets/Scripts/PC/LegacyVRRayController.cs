...
public class LegacyVRRayController : MonoBehaviour {
    ...
    private void PullTowards(Magnet target) {
        ...
        Debug.Log(len);
        if (true) {
            pcon.rigid.AddExplosionForce(PULL_STR * Mathf.Clamp01(len / (target.radius * 5f)) * -1, target.transform.position, len * 2f);
        }
        else {
